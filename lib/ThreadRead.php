...
class ThreadRead extends Thread {
    ...
    public function downloadDat() {
        ...
        if (P2BbsType::isHostMachiBbs ($this->host)) {
            ...
            // 2ch はAPI経由で落とす
            } elseif (P2BbsType::isHost2chs ($this->host) && !P2BbsType::isNotUse2chsAPI ($this->host) && $_conf['2chapi_use'] && empty ($_GET['olddat'])) {
    ...
    protected function _downloadDat2chAPI($uaMona, $SID2ch, $from_bytes) {
        ...
        $url= http_build_url(array(
            ...
            "host" => P2BbsType::isHost5ch($this->host)?"api.5ch.net":"api.2ch.net",
    ...
    public function get2chDatError($code = null) {
        ...
        $reason = null;
        if (P2BbsType::isHost2chs ($this->host) || P2BbsType::isHostVip2ch ($this->host)) {
            if ($code == '302') {
        ...
        if (! $reason) {
            try {
                ...
                // ヘッダ
                $req->setHeader ('User-Agent', P2Commun::getP2UA(false,P2BbsType::isHost2chs($this->host))); // ここは、"Monazilla/" をつけるとNG
    ...
    public function previewOne() {
        ...
        if (is_readable ($this->keydat)) {
            ...
            if (P2BbsType::isHostBe2chNet ($this->host)) {
    ...
    public function scanOriginalHosts() {
        if (P2BbsType::isHost2chs ($this->host) && file_exists ($this->keydat) && ($dat = file_get_contents ($this->keydat))) {
