...
$ini_url_text = '';
if (!$aThreadList->spmode) {
    if (P2BbsType::isHostJbbsShitaraba($aThreadList->host)) { // したらば
        $ini_url_text = "http://{$aThreadList->host}/bbs/read.cgi?BBS={$aThreadList->bbs}&KEY=";
    } elseif (P2BbsType::isHostMachiBbs($aThreadList->host)) { // まちBBS
        $ini_url_text = "http://{$aThreadList->host}/bbs/read.pl?BBS={$aThreadList->bbs}&KEY=";
    } elseif (P2BbsType::isHostMachiBbsNet($aThreadList->host)) { // まちビねっと
